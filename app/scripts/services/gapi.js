...
angular.module('gapi', [])
  ...
  .factory('GoAuth', function($http, GoogleApp){
    var GoAuth = {
    
      _buildAuthUrl: function(options){
        var url = 'https://accounts.google.com/o/oauth2/auth?';
        var params = [];
        for(var opt in options){
          params.push([opt,options[opt]].join('='));
        }
      ...
      authorize: function(options){
        window.location = this._buildAuthUrl(options);
      },

      validateToken: function(access_token, success, error){
        var url = 'https://www.googleapis.com/oauth2/v1/tokeninfo';
        $http({method: 'GET', url: url, params:{access_token: access_token}}).success(success).error(error);
      }
  ...
  .factory('GAPI', function ($q, $http, GoAuth, GoogleApp) {
    ...
    GAPI.init = function (access_token, success, error) {
      GAPI.app.access_token = access_token;
      if(access_token){
        GoAuth.validateToken(GAPI.app.access_token, success, error);
      } else {
        GoAuth.authorize(GoogleApp);   
      }
    }
